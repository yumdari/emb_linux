...
irqreturn_t sw_isr(int irq, void *unuse)
{
	...
	//윗줄 주석을 하지 않는다면 키가 안눌려지면 1초에 한번씩 리턴을 하다가 1초가 되기 전에 스위치를 누르면
	//바로 리턴을 하게 된다
	return IRQ_HANDLED;
}
...
static ssize_t ledkey_read(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
	...
	if(!(filp->f_flags & O_NONBLOCK))
	{
		//if(sw_no == 0)	
			...
			//가장 기본 블로킹 (별도의 스위치 입력이 없으면 항시 블로킹 상태)
			//wait_event_interruptible(WaitQueue_Read,sw_no);			 //잠들긴 하지만 이벤트에 의해서 깨어난다
			//깨어나는 이벤트는 sw_no 이며, sw_no == 1 일때 깨어남
			//따라서 if문을 제거할 수 있다
			//wait_event_interruptible_timeout(WaitQueue_Read,sw_no,100); //100*1/100Hz = 1s
			if(sw_no == 0)
				return 0;
	}
	kbuf = (char)sw_no;
